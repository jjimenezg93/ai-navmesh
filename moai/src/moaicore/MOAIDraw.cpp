...
void MOAIDraw::DrawPolygon(const std::vector<USVec2D>& points)
{
    MOAIGfxDevice& gfxDevice = MOAIGfxDevice::Get();
    gfxDevice.BeginPrim ( GL_LINE_LOOP );
	
    for (std::vector<USVec2D>::const_iterator it = points.begin(); it < points.end(); ++it)
    {
        const USVec2D& v = *it;
        gfxDevice.WriteVtx(v.mX, v.mY, 0.0f);
        gfxDevice.WriteFinalColor4b();
    }
...
void MOAIDraw::DrawPolygonFilled(const std::vector<USVec2D>& points)
{
    MOAIGfxDevice& gfxDevice = MOAIGfxDevice::Get ();
	
	gfxDevice.BeginPrim ( GL_POLYGON );
