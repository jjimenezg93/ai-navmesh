...
int	MOAILayer::_setDrawDebug (lua_State* L)
{
	MOAI_LUA_SETUP ( MOAILayer, "UU" )

	self->mDrawDebug.Set( *self, state.GetLuaObject <MOAIDrawDebug>(2, true));
...
//----------------------------------------------------------------//
void MOAILayer::InsertProp(MOAIProp* prop)
{
	if (!prop) return;
	if (prop == this) return;

	AffirmPartition ();
	mPartition->InsertProp ( *prop );
	prop->ScheduleUpdate ();
}

void MOAILayer::RemoveProp(MOAIProp* prop)
{
	if (!prop) return;
	if (prop == this) return;
