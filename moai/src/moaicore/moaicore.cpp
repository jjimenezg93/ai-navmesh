...
void moaicore::InitGlobals ( MOAIGlobals* globals ) {
    ...
    REGISTER_LUA_CLASS(MOAIEntity)
	REGISTER_LUA_CLASS(MOAIEntity2D)
	REGISTER_LUA_CLASS(MOAIDrawDebug)

	if (sHostContext)
	{
		sHostContext->InitGlobals(globals);
	}
