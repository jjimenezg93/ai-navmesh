...
public:
    ...
    //----------------------------------------------------------------//
    USMetaVec2D operator += ( const USMetaVec2D& v ) {
        mX = this->mX + v.mX;
        mY = this->mY + v.mY;
        return *this;
    }
    ...
    USMetaVec2D operator * (const float& n) const {
        USMetaVec2D < TYPE > result(*this);
        result.Scale(n);
        return result;
    }

    USMetaVec2D operator *= (const float& n) {
        Scale(n);
        return *this;
    }
    ...
    USMetaVec2D(const USMetaVec3D<TYPE>& vec) : mX(vec.mX), mY(vec.mY) {}
