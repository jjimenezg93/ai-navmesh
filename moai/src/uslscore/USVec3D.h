...
public:
    ...
    USMetaVec3D operator += ( const USMetaVec3D& v ) {
        mX = this->mX + v.mX;
        mY = this->mY + v.mY;
        mZ = this->mZ + v.mZ;
        return *this;
    ...
    USMetaVec3D operator * (const float& n) const
    {
        USMetaVec3D < TYPE > result(*this);
        result.Scale(n);
        return result;
    }

    USMetaVec3D operator *= (const float& n)
    {
        Scale(n);
        return *this;
    }
    ...
    USMetaVec3D(const USMetaVec2D<TYPE>& vec2D, TYPE z = 0) : mX(vec2D.mX), mY(vec2D.mY), mZ(z) {}
